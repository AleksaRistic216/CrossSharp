...
partial class FormSDL
{
    ...
    static IntPtr CreateWindow(string title, int width, int height) => CreateWindowCore(title, width, height);
    ...
    static IntPtr CreateWindowCore(string title, int width, int height)
    {
        ...
        if (appConfig.FormsStyle is FormStyle.CrossSharp)
        {
