...
public class ModularForm : FormBase<IModularFormFactory>, IModularForm
{
    ...
    {
        get => ((IModularForm)Implementation).TopNavigationVisible;
        set => ((IModularForm)Implementation).TopNavigationVisible = value;
    }

    public void AddPage(object identifier, Type pageType) =>
        ((IModularForm)Implementation).AddPage(identifier, pageType);

    public void NavigateToPage(object identifier) => ((IModularForm)Implementation).NavigateToPage(identifier);
    ...
    public void RegisterContentSingleton<T>(T instance, bool overrideExisting = false)
        where T : class => ((IModularForm)Implementation).RegisterContentSingleton(instance, overrideExisting);
}
