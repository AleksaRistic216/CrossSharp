...
static class ServicesPool {
    ...
    internal static void AddSingleton<T>(T instance) where T : class {
        Type implementationType = typeof(T);
        if (!_services.TryAdd(implementationType, instance))
            throw new InvalidOperationException($"Service for type {implementationType.FullName} is already registered.");
    }
    internal static void AddSingleton<TInterface, TImplementation>()
    ...
    internal static TInterface GetSingleton<TInterface>() where TInterface : class {
        Type interfaceType = typeof(TInterface);
        if (_services.TryGetValue(interfaceType, out object service)) {
            return service as TInterface ?? throw new InvalidCastException($"Registered service cannot be cast to type {interfaceType.FullName}.");
        }
